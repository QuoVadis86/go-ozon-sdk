...
type V5FbsPostingProductExemplarValidateV5RequestProductExemplarMark struct {
	...
	MarkType string `json:"mark_type"` // 标志代码类型： - `mandatory_mark` — “诚实标志”（Chestny ZNAK）强制性标志； - `jw_uin` — 珠宝制品的唯 一 识别编号； - `imei` — 移动设备 IMEI。
}
...
type V5FbsPostingProductExemplarStatusV5ResponseProductExemplarMark struct {
	MarkType string `json:"mark_type"` // 标志代码类型： - `mandatory_mark` — “诚实标志”（Chestny ZNAK）强制性标志； - `jw_uin` — 珠宝制品的唯 一 识别编号； - `imei` — 移动设备 IMEI。
	CheckStatus string `json:"check_status"` // 检查状态： - `processing`——正在检查中； - `failed`——系统未能成功处理请求； - `passed`——订单已准备好进行备货。
