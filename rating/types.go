...
type ListFBSRatingIndexPostingsV1ResponseError struct {
	ChargePriceCurrencyCode string `json:"charge_price_currency_code"` // 错误处理费用的币种代码： - `RUB`——俄罗斯卢布， - `BYN`——白俄罗斯卢布， - `KZT`——坚戈， - `EUR`——欧元， - `USD`——美元， - `CNY`——人民币。
	...
	ProductPriceCurrencyCode string `json:"product_price_currency_code"` // 商品价值的币种代码： - `RUB`——俄罗斯卢布， - `BYN`——白俄罗斯卢布， - `KZT`——坚戈， - `EUR`——欧元， - `USD`——美元， - `CNY`——人民币。
	ChargePercent float64 `json:"charge_percent"` // 处理费用占货件价值的百分比。
	ChargePrice float64 `json:"charge_price"` // 错误处理费用金额。
	DeliverySchema string `json:"delivery_schema"` // 配送方案： - `FBS`, - `rFBS`, - `erFBS`.
