...
type V1ItemResponse struct {
	...
	PlacementZone string `json:"placement_zone"` // 商品存放区域： - `UNSPECIFIED`——未指定； - `CLOSED_ZONE`——封闭区域； - `DANGEROUS_GOODS`——2–4 类危险品； - `PRODUCTS`——食品； - `SORT`——可分拣商品； -...
...
type V1ArchiveDeclineReason struct {
	Code string `json:"code"` // 拒绝交货原因代码： - `DECLINE_REASON_CODE_UNSPECIFIED`：未指定； - `CANNOT_CREATE_SUPPLY_ON_TPF`：无法在3PF创建交货； - `DROP_OFF_POINT_CLOSED`...
