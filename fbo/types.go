...
const (
	...
	ErrorsOUTOFAssortment            Errors = "OUT_OF_ASSORTMENT"             // 未找到商品；
	ErrorsInvalid                    Errors = "INVALID"                       // 商品未创建；
	ErrorsIncompatibleWarehouse      Errors = "INCOMPATIBLE_WAREHOUSE"        // 仓库标识符错误；
	ErrorsInvalidBarcode             Errors = "INVALID_BARCODE"               // 未指定条形码；
	ErrorsMultiplicity               Errors = "MULTIPLICITY"                  // 商品数量不是所需批量的数倍；
	ErrorsNOPrice                    Errors = "NO_PRICE"                      // 未指定价格；
	ErrorsBanned                     Errors = "BANNED"                        // 商品不可在选定仓库销售或交货；
	ErrorsZeroQuantity               Errors = "ZERO_QUANTITY"                 // 交货商品数量为0；
	ErrorsQuantityGreaterThenMAX     Errors = "QUANTITY_GREATER_THEN_MAX"     // 单个SKU商品数量超过最大值；
