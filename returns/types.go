...
type V2Product struct {
	...
	CurrencyCode string `json:"currency_code"` // 您的价格所使用的货币。与您在个人中心中设置的货币相匹配。 可能的值: - `RUB` — 俄罗斯卢布, - `BYN` — 白俄罗斯卢布, - `KZT` — 坚戈, - `EUR` — 欧元, - `USD` — 美元, - `CNY` ...
...
type GetReturnsListRequestFilter struct {
	...
	VisualStatusName string `json:"visual_status_name"` // 根据退货状态筛选： - `DisputeOpened` — 与买家有争议； - `OnSellerApproval` — 等待卖家批准； - `ArrivedAtReturnPlace` — 到达取货点； - `OnSellerClarif...
...
type GetReturnsListResponseCompensationStatus struct {
	...
	SysName string `json:"sys_name"` // 状态的系统名称： - `Sent`——已发送； - `Received`——已收到； - `Canceled`——已取消； - `DecompensationSent`——已进行赔偿返还。
}
