...
// 根据标识符获取商品信息
// Note: 最多可通过 `offer_id`、`product_id` 和 `sku` 传递总计不超过 1000个商品标识符。
...
// 创建或更新商品
// Note: 请使用 [/v4/product/info/limit](#operation/ProductAPI_GetUploadQuota)。
// Note: 无法接收没有条形码的商品。
// Note: 如需上传Ozon Select主图： 1. 请检查方法[/v1/description-category/attribute](#operation/DescriptionCategoryAPI_GetAttributes) 的响应中是否返...
...
// 上传或更新商品图片
// Note: 请使用 [/v3/product/info/list](#operation/ProductAPI_GetProductInfoList) 方法获取信息 —— 它显示了当前图像的顺序和组成。
// Note: 每个商品上传多达30张图片。
...
// Note: 如需完整更新全部特征，请使用/v3/product/import。
