...
type Postingv3GetFbsPostingListRequestFilter struct {
	...
	FbpFilter             FbpFilter           `json:"fbpFilter"`          // 从合作伙伴仓库（FBP）发货时的货件筛选器： - `ALL` — 响应中将显示所有符合其他筛选器条件的货件； - `ONLY` — 仅显示FBP货件； - `WITHOUT` — 显示除FBP外的所有货件。 默认值为 `ALL`。
	Since                 string              `json:"since"`              // 应收到货件清单时间段的开始日期。 UTC模式: YYYY-MM-DDThh:mm:ssZ. 例子: 2019-08-24T14:15:22Z.
	To                    string              `json:"to"`                 // 应收到货件清单时间段的结束日期。 UTC模式： YYYY-MM-DDThh:mm:ssZ. 例子： 2019-08-24T14:15:22Z.
...
type PostingFinancialDataProduct struct {
	...
	CurrencyCode            enum.CurrencyCode `json:"currency_code"`             // 价格货币，其与个人中心中设置的币种相匹配。 可能的值： - `RUB` — 俄罗斯卢布， - `BYN` — 白俄罗斯卢布， - `KZT` — 坚戈， - `EUR` — 欧元， - `USD` — 美元， - `CNY` — 元。
...
type PostingV4PostingFbsUnfulfilledListResponsePostingsRequirements struct {
	...
	ProductsRequiringMandatoryMark []string `json:"products_requiring_mandatory_mark"` // S需要传递“诚实标志”标识的商品标识符（SKU）列表。 在备货货件前，请通过方法[/v6/fbs/posting/product/exemplar/set](#operation/PostingAPI_FbsPostingProductEx...
...
type V4PostingProductDetailWithoutDimensions struct {
	...
	SKU           int64             `json:"sku"`            // Ozon系统中的商品ID — SKU.
	CurrencyCode  enum.CurrencyCode `json:"currency_code"`  // 价格币种。这与您在个人中心中设置的币种 一 致。 可能的值： - `RUB` — 卢布， - `BYN` — 白俄罗斯卢布， - `KZT` — 坚戈， - `EUR` — 欧元， - `USD` — 美元， - `CNY` — 元。
...
type V3PostingProductDetail struct {
	...
	CurrencyCode  enum.CurrencyCode `json:"currency_code"` // 价格显示的货币，其与个人中心中设置的币种相匹配。 -`RUB` — 俄罗斯卢布， - `BYN` — 白俄罗斯卢布， - `KZT` — 坚戈， - `EUR` — 欧元， - `USD` — 美元， - `CNY` — 元。
	Quantity      int32             `json:"quantity"`      // 商品数量。
	SKU           int64             `json:"sku"`           // Ozon 系统中的商品标识符（SKU）。
