...
const (
	...
	CancellationInitiatorV_4  CancellationInitiator = "配送服务商"
)
...
const (
	CancellationTypeSeller   CancellationType = "seller"   // 由卖家取消；
	CancellationTypeClient   CancellationType = "client"   // 或`customer`——由买家取消；
	CancellationTypeOzon     CancellationType = "ozon"     // 由Ozon取消；
	CancellationTypeSystem   CancellationType = "system"   // 由系统取消；
	CancellationTypeDelivery CancellationType = "delivery" // 由配送服务取消
)
...
const (
	...
	AvailableActionsAwaitingDelivery       AvailableActions = "awaiting_delivery"         // 转为“等待发运”状态；
	AvailableActionsCanCreateChat          AvailableActions = "can_create_chat"           // 与买家开启聊天；
	AvailableActionsCancel                 AvailableActions = "cancel"                    // 取消货件；
	AvailableActionsClickTrackNumber       AvailableActions = "click_track_number"        // 在个人中心通过追踪号查看状态历史；
	AvailableActionsCustomerPhoneAvailable AvailableActions = "customer_phone_available"  // 获取买家电话号码；
	AvailableActionsHasWeightProducts      AvailableActions = "has_weight_products"       // 货件中包含以重量结算；
	...
	AvailableActionsNonIntDelivered        AvailableActions = "non_int_delivered"         // 转为“可能已收”状态；
	AvailableActionsNonIntDelivering       AvailableActions = "non_int_delivering"        // 转为“运输中”状态；
	AvailableActionsNonIntLastMile         AvailableActions = "non_int_last_mile"         // 转为“快递员派件中”状态；
	AvailableActionsProductCancel          AvailableActions = "product_cancel"            // 取消部分货件中的商品；
	AvailableActionsSetCutoff              AvailableActions = "set_cutoff"                // 需要指定发货日期，请使用方法[/v1/posting/cutoff/set](#operation/PostingAPI_SetPostingCutoff)；
	AvailableActionsSetTimeslot            AvailableActions = "set_timeslot"              // 修改买家的送货时间；
	AvailableActionsSetTrackNumber         AvailableActions = "set_track_number"          // 指定或更改追踪号；
	AvailableActionsShipAsyncInProcess     AvailableActions = "ship_async_in_process"     // 货件备货中；
	AvailableActionsShipAsyncRetry         AvailableActions = "ship_async_retry"          // 发生错误后重新发货；
	AvailableActionsShipAsync              AvailableActions = "ship_async"                // 备货货件；
	AvailableActionsShipWithAdditionalInfo AvailableActions = "ship_with_additional_info" // 需要填写额外信息；
...
const (
	...
	SubstatusPostingTransferringToDelivery      Substatus = "posting_transferring_to_delivery"       // (`status=awaiting_deliver`) —— 移交给快递
...
// 排序方向： - `ASC`——升序， - `DESC`——降序。
type V1AssemblyFbsProductListRequestSortDir string

const (
	V1AssemblyFbsProductListRequestSortDirASC  V1AssemblyFbsProductListRequestSortDir = "ASC"
	V1AssemblyFbsProductListRequestSortDirDesc V1AssemblyFbsProductListRequestSortDir = "DESC"
)

type V1AssemblyFbsProductListRequest struct {
	Offset  int64                                  `json:"offset"` // 在响应中将被跳过的项目数量。例如，如果 `offset = 10`，则响应将从第 11 个找到的项目开始。
	SortDir V1AssemblyFbsProductListRequestSortDir `json:"sort_dir"`
...
// 排序方向： - `ASC`——升序， - `DESC`——降序。
type V1AssemblyFbsPostingListRequestSortDir string

const (
	V1AssemblyFbsPostingListRequestSortDirASC  V1AssemblyFbsPostingListRequestSortDir = "ASC"
	V1AssemblyFbsPostingListRequestSortDirDesc V1AssemblyFbsPostingListRequestSortDir = "DESC"
)
...
type V1AssemblyFbsPostingListRequest struct {
	...
	Limit   int64                                  `json:"limit"` // 每页显示的数量。
	SortDir V1AssemblyFbsPostingListRequestSortDir `json:"sort_dir"`
}
...
// 排序方向： - `ASC`——升序； - `DESC`——降序。
type PostingV4PostingFbsListRequestSortDir string

const (
	PostingV4PostingFbsListRequestSortDirASC  PostingV4PostingFbsListRequestSortDir = "ASC"
	PostingV4PostingFbsListRequestSortDirDesc PostingV4PostingFbsListRequestSortDir = "DESC"
)
...
// 排序方向： - `ASC`——升序； - `DESC`——降序。
type PostingV4PostingFbsUnfulfilledListRequestSortDir string

const (
	PostingV4PostingFbsUnfulfilledListRequestSortDirASC  PostingV4PostingFbsUnfulfilledListRequestSortDir = "ASC"
	PostingV4PostingFbsUnfulfilledListRequestSortDirDesc PostingV4PostingFbsUnfulfilledListRequestSortDir = "DESC"
)
...
type PostingV4PostingFbsListRequest struct {
	...
	SortDir  PostingV4PostingFbsListRequestSortDir `json:"sort_dir"`
...
type PostingV4PostingFbsUnfulfilledListRequest struct {
	...
	Limit    int64                                            `json:"limit"` // 响应中返回的值数量。
	SortDir  PostingV4PostingFbsUnfulfilledListRequestSortDir `json:"sort_dir"`
