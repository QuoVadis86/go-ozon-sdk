...
type PostingCancelReason struct {
	TypeID string `json:"type_id"` // 取消货件ID： - `buyer` — 买家， - `seller` — 卖家。
...
type V3PostingProductDetail struct {
	...
	CurrencyCode string `json:"currency_code"` // 价格显示的货币，其与个人中心中设置的币种相匹配。 -`RUB` — 俄罗斯卢布， - `BYN` — 白俄罗斯卢布， - `KZT` — 坚戈， - `EUR` — 欧元， - `USD` — 美元， - `CNY` — 元。
	SKU int64 `json:"sku"` // Ozon 系统中的商品标识符（SKU）。
...
type Fbsv4PostingProductDetailWithoutDimensions struct {
	...
	CurrencyCode string `json:"currency_code"` // 价格币种。这与您在个人中心中设置的币种 一 致。 可能的值： - `RUB` — 卢布， - `BYN` — 白俄罗斯卢布， - `KZT` — 坚戈， - `EUR` — 欧元， - `USD` — 美元， - `CNY` — 元。
...
type PostingV4PostingFbsListResponsePostingsAnalyticsData struct {
	...
	PaymentTypeGroupName string `json:"payment_type_group_name"` // 支付方法： - `在线银行卡支付`； - `Ozon Bank银行卡`； - `取货时从Ozon卡自动扣款`； - `收货时用已保存的银行卡支付`； - `快速支付系统`； - `Ozon分期付款`； - `支付至结算账户`； - `Sbe...
...
type Postingv3GetFbsPostingListRequestFilter struct {
	...
	Status string `json:"status"` // 货件运输状态： - `awaiting_registration` — 等待注册， - `acceptance_in_progress` — 正在验收， - `awaiting_approve` — 等待确认， - `awaiting_pa...
	...
	FbpFilter string `json:"fbpFilter"` // 从合作伙伴仓库（FBP）发货时的货件筛选器： - `ALL` — 响应中将显示所有符合其他筛选器条件的货件； - `ONLY` — 仅显示FBP货件； - `WITHOUT` — 显示除FBP外的所有货件。 默认值为 `ALL`。
...
type V3FbsPostingAnalyticsData struct {
	PaymentTypeGroupName string `json:"payment_type_group_name"` // 付款方法： - `在线银行卡支付`， - `Ozon银行卡`， - `取货时自动从Ozon银行卡收费`， - `收货时从已保存的银行卡收费`， - `快速支付系统`， - `Ozon分期付款`， - `支付至结算账户`， - `SberPa...
...
type V3Cancellation struct {
	...
	CancellationInitiator string `json:"cancellation_initiator"` // 取消货运的发起者： - `卖家`, - `客户` 或`买家`, - `Ozon`, - `系统`, - `配送服务`。
	CancellationType string `json:"cancellation_type"` // 货运取消类型： - `seller` — 卖家取消； - `client` 或 `customer` — 买家取消； - `ozon` — Ozon取消； - `system`— 系统取消； - `delivery` — 配送服务取消。
}
...
type V3FbsPostingProduct struct {
	...
	CurrencyCode string `json:"currency_code"` // 价格货币，其与个人中心中设置的币种相匹配。 可能的值： - `RUB` — 俄罗斯卢布， - `BYN` — 白俄罗斯卢布， - `KZT` — 坚戈， - `EUR` — 欧元， - `USD` — 美元， - `CNY` — 元。
...
type V3FbsPosting struct {
	...
	TPLIntegrationType string `json:"tpl_integration_type"` // 快递服务集成类型： - `ozon` —— Ozon 快递服务。 - `3pl_tracking` —— 集成服务快递。 - `non_integrated` —— 第三方物流服务。 - `aggregator` —— 通过Ozon合作物流...
	...
	Status string `json:"status"` // 货运状态: - `acceptance_in_progress` —— 正在验收， - `arbitration` —— 仲裁， - `awaiting_approve` —— 等待确认， - `awaiting_deliver` —— 等...
	...
	Substatus string `json:"substatus"` // 发货子状态： - `posting_acceptance_in_progress` —— 正在验收， - `posting_in_arbitration` —— 仲裁， - `posting_created` —— 已创建， - `post...
...
type PostingV4PostingFbsListResponsePostingsCancellation struct {
	...
	CancellationInitiator string `json:"cancellation_initiator"` // 取消发起方： - `卖家`， - `客户`， - `买家`， - `Ozon`， - `系统`， - `配送服务商`。
	CancellationType string `json:"cancellation_type"` // 取消类型： - `seller`——由卖家取消； - `client`或`customer`——由买家取消； - `ozon`——由Ozon取消； - `system`——由系统取消； - `delivery`——由配送服务取消。
...
type PostingV4PostingFbsListResponsePostings struct {
	...
	TarifficationSteps []PostingV4PostingFbsListResponsePostingsTarifficationStep `json:"tariffication_steps"` // 计费阶段。
	PostingNumber string `json:"posting_number"` // 货件编号。
	PrrOption string `json:"prr_option"` // 装卸服务代码： - `lift`——通过电梯搬运上楼； - `stairs`——通过楼梯搬运上楼； - `none`——买家拒绝该服务，无需将商品搬上楼； - `delivery_default`——配送费用已包含在价格中，根据要约条款需将...
	...
	Status string `json:"status"` // 货件状态： - `acceptance_in_progress`——接收中； - `arbitration`——仲裁； - `awaiting_approve`——等待确认； - `awaiting_deliver`——等待发运； - `a...
	Substatus string `json:"substatus"` // 货件子状态： - `posting_acceptance_in_progress`——验收中； - `posting_in_arbitration`——仲裁； - `posting_created`——已创建； - `posting_in_...
	TrackingNumber string `json:"tracking_number"` // 货件跟踪号码。
	DeliverySchema string `json:"delivery_schema"` // 配送模式： - `SDS`——统一SKU标识符； - `FBO`——从Ozon仓库销售的商品标识符； - `FBS`——从FBS仓库销售的商品标识符； - `Crossborder`——从境外销售的商品标识符。
	...
	TPLIntegrationType string `json:"tpl_integration_type"` // 与配送服务的集成类型： - `ozon`——由Ozon配送； - `3pl_tracking`——由已集成服务配送； - `non_integrated`——由第三方服务配送； - `aggregator`——通过Ozon合作伙伴配送； -...
}
...
type ResultError struct {
	...
	Status string `json:"status"` // 错误类型： - `warning` — 提醒； - `critical` — 严重错误。
}
...
type RelatedPostingCancelReasons struct {
	...
	TypeID string `json:"type_id"` // 取消货运提出方： - `buyer` — 买家， - `seller` — 卖家。
}
...
type Postingv3GetFbsPostingUnfulfilledListRequestFilter struct {
	...
	FbpFilter string `json:"fbpFilter"` // 从合作伙伴仓库（FBP）发货时的货件筛选器： - `ALL` — 响应中将显示所有符合其他筛选器条件的货件； - `ONLY` — 仅显示FBP货件； - `WITHOUT` — 显示除FBP外的所有货件。 默认值为 `ALL`。
	ProviderID []int64 `json:"provider_id"` // 快递服务ID。按照运输方式筛选。可以使用方法 [/v1/delivery-method/list](#operation/WarehouseAPI_DeliveryMethodList)获取。
	Status string `json:"status"` // 货件运输状态： - `acceptance_in_progress` — 正在验收， - `awaiting_approve` — 等待确认， - `awaiting_packaging` — 等待包装， - `awaiting_regis...
}
...
type V3FbsPostingDetail struct {
	...
	OrderNumber string `json:"order_number"` // 货件所属的订单号。
	LegalInfo V2FboSinglePostingLegalInfo `json:"legal_info"`
	TPLIntegrationType string `json:"tpl_integration_type"` // 快递服务集成类型： - `ozon` —— 通过Ozon物流的快递。 - `aggregator` —— 外部服务快递，Ozon注册订单。 - `3pl_tracking` —— 外部服务快递，卖家注册订单。 - `non_integrat...
	Tariffication V3FbsTariffication `json:"tariffication"`
	TarifficationSteps []PostingV4PostingFbsListResponsePostingsTarifficationStep `json:"tariffication_steps"` // 计费阶段。
	Requirements V3FbsPostingRequirementsV3 `json:"requirements"`
	ShipmentDateWithoutDelay string `json:"shipment_date_without_delay"` // 不逾期发运日期和时间。
	...
	PreviousSubstatus string `json:"previous_substatus"` // 货件的前一个子状态。可能的取值： - `posting_acceptance_in_progress` —— 正在验收， - `posting_in_arbitration` —— 仲裁， - `posting_created` —— 已创...
	AnalyticsData V3FbsPostingAnalyticsData `json:"analytics_data"`
	DeliveringDate string `json:"delivering_date"` // 货件交付物流的时间。
	Substatus string `json:"substatus"` // 发货子状态： - `posting_acceptance_in_progress` —— 正在验收， - `posting_in_arbitration` —— 仲裁， - `posting_created` —— 已创建， - `post...
