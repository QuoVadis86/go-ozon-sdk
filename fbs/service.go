...
// 打印标签
func (s *Service) PostingFBSPackageLabel(ctx context.Context, req *PostingPostingFBSPackageLabelRequest) error {
...
// 搜集订单 (第4方案)
// Note: 请使用/v3/posting/fbs/get方式来检查，订单是否已完成备货
// Note: 每个元素都可以包含多个`products`和货物
// Note: 不可以放在一个包装里
func (s *Service) ShipFbsPostingV4(ctx context.Context, req *Fbsv4FbsPostingShipV4Request) (*Fbsv4FbsPostingShipV4Response, error) {
...
// 货位标签
func (s *Service) PostingFBSActGetContainerLabels(ctx context.Context, req *PostingPostingFBSActGetContainerLabelsRequest) error {
...
// Note: 在更改状态前，请使用 /v3/posting/fbs/get 方法检查当前货件状
...
// 取消某些商品发货
// Note: 请使用该方法。 为了在使用FBS或rFBS模式时获取取消原因的标识符`cancel_reason_id`，请使用方法[/v2/po
// Note: 无法从货件中发送部分产品，请使用该方法
// Note: 为了在使用FBS或rFBS模式时获取取消原因的标识符`cancel_reason_id`，请使用方法[/v2/posting/fbs/cancel-reason
