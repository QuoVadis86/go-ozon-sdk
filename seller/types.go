...
type SellerInfoResponseCompany struct {
	Currency string `json:"currency"` // 货币： - `RUB`——俄罗斯卢布； - `EUR`——欧元； - `USD`——美元； - `CNY`——人民币； - `BYN`——白俄罗斯卢布； - `KZT`——坚戈； - `KGS`——吉尔吉斯斯坦索姆。
...
type V1SellerOzonLogisticsInfoResponse struct {
	AvailableSchemas []SellerOzonLogisticsInfoResponseAvailableSchemasEnum `json:"available_schemas"` // 可用模式类型： - `UNKNOWN`——未指定； - `FBO`——从Ozon仓库配送； - `FBS`——从自有仓库配送。
