...
type ProductImportProductsPricesRequestPrice struct {
	PriceStrategyEnabled string `json:"price_strategy_enabled"` // 用于自动应用价格策略的属性： - `ENABLED` — 启用; - `DISABLED` — 关闭; - `UNKNOWN` — 不做任何更改，默认赋值。 如果您之前已启用价格策略的自动应用，并且不希望关闭，请在后续请求中赋值 `UNKN...
	ProductID int64 `json:"product_id"` // Ozon系统中商品的标识符 — `product_id`。
	AutoActionEnabled string `json:"auto_action_enabled"` // 启用和禁用自动应用活动的属性： - `ENABLED` — 启用; - `DISABLED` — 关闭; - `UNKNOWN` — 不做任何更改，默认赋值。 例如，如果你以前启用了活动的自动应用，并且不想关闭它，请提交`UNKNOWN`。...
	CurrencyCode string `json:"currency_code"` // 您的价格的货币。提交的数值必须与你个人主页设置中的货币相匹配。默认情况下，会提交 `RUB` — 俄罗斯卢布。 例如，如果您设置的相互结算的货币是人民币，请提交数值 `CNY`，否则将返回错误。 可填的数值： - `RUB` — 俄罗斯卢布...
...
type ItemPricev5 struct {
	...
	CurrencyCode string `json:"currency_code"` // 您的定价货币（与个人中心设置中指定的货币相同）。 可能的值： - `RUB` — 俄罗斯卢布， - `BYN` — 白俄罗斯卢布， - `KZT` — 坚戈， - `EUR` — 欧元， - `USD` — 美元， - `CNY` — 人民...
...
type GetProductInfoPricesResponseItemPriceIndexes struct {
	ColorIndex string `json:"color_index"` // 最终价格指数： - `WITHOUT_INDEX` — 无价格指数， - `SUPER` — 超级有利, - `GREEN` — 有利， - `YELLOW` — 中等， - `RED` — 不利。
...
type GetProductInfoStocksResponseStock struct {
	...
	Type string `json:"type_"` // 仓库类型： - `fbs`——卖家仓库，由 Ozon 配送； - `rfbs`——卖家仓库，由卖家自行配送； - `fbo`——Ozon 仓库； - `fbp`——合作伙伴仓库。
