package prices

// 包装类型： - `SHIPMENT_TYPE_GENERAL` — 普通商品； - `SHIPMENT_TYPE_BOX` — 盒子； - `SHIPMENT_TYPE_PALLET` — 托盘。
type StockShipmentType string

// 仓库类型：
type GetProductInfoStocksResponseStockTypeEnum string
const (
	GetProductInfoStocksResponseStockTypeEnum_Fbs GetProductInfoStocksResponseStockTypeEnum = "fbs"
	GetProductInfoStocksResponseStockTypeEnum_Rfbs GetProductInfoStocksResponseStockTypeEnum = "rfbs"
	GetProductInfoStocksResponseStockTypeEnum_Fbo GetProductInfoStocksResponseStockTypeEnum = "fbo"
	GetProductInfoStocksResponseStockTypeEnum_Fbp GetProductInfoStocksResponseStockTypeEnum = "fbp"
)

type GetProductInfoStocksResponseStock struct {
	Type GetProductInfoStocksResponseStockTypeEnum `json:"type_"` // 仓库类型： - `fbs`——卖家仓库，由 Ozon 配送； - `rfbs`——卖家仓库，由卖家自行配送； - `fbo`——Ozon 仓库； - `fbp`——合作伙伴仓库。
	WarehouseIds []int64 `json:"warehouse_ids"` // 存放或曾经存放该商品的仓库标识符。
	Present int32 `json:"present"` // 现在在仓库中。
	Reserved int32 `json:"reserved"` // 已预定。
	ShipmentType StockShipmentType `json:"shipment_type"`
	SKU int64 `json:"sku"` // Ozon系统中的商品识别符——SKU。
}

type V4GetProductInfoStocksResponseItem struct {
	OfferID string `json:"offer_id"` // 卖家系统中的商品识别符——货号。
	ProductID int64 `json:"product_id"` // 商品识别符。
	Stocks []GetProductInfoStocksResponseStock `json:"stocks"` // 库存信息。
}

// 用于自动应用价格策略的属性：
type ProductImportProductsPricesRequestPricePriceStrategyEnabledEnum string
const (
	ProductImportProductsPricesRequestPricePriceStrategyEnabledEnum_ENABLED ProductImportProductsPricesRequestPricePriceStrategyEnabledEnum = "ENABLED"
	ProductImportProductsPricesRequestPricePriceStrategyEnabledEnum_DISABLED ProductImportProductsPricesRequestPricePriceStrategyEnabledEnum = "DISABLED"
	ProductImportProductsPricesRequestPricePriceStrategyEnabledEnum_UNKNOWN ProductImportProductsPricesRequestPricePriceStrategyEnabledEnum = "UNKNOWN"
	ProductImportProductsPricesRequestPricePriceStrategyEnabledEnum_MinPrice ProductImportProductsPricesRequestPricePriceStrategyEnabledEnum = "min_price"
)

// 商品增值税税率：
type ProductImportProductsPricesRequestPriceVatEnum string
const (
	ProductImportProductsPricesRequestPriceVatEnum_V005 ProductImportProductsPricesRequestPriceVatEnum = "0.05"
	ProductImportProductsPricesRequestPriceVatEnum_V007 ProductImportProductsPricesRequestPriceVatEnum = "0.07"
	ProductImportProductsPricesRequestPriceVatEnum_V01 ProductImportProductsPricesRequestPriceVatEnum = "0.1"
	ProductImportProductsPricesRequestPriceVatEnum_V02 ProductImportProductsPricesRequestPriceVatEnum = "0.2"
	ProductImportProductsPricesRequestPriceVatEnum_V022 ProductImportProductsPricesRequestPriceVatEnum = "0.22"
)

// 启用和禁用自动应用活动的属性：
type ProductImportProductsPricesRequestPriceAutoActionEnabledEnum string
const (
	ProductImportProductsPricesRequestPriceAutoActionEnabledEnum_ENABLED ProductImportProductsPricesRequestPriceAutoActionEnabledEnum = "ENABLED"
	ProductImportProductsPricesRequestPriceAutoActionEnabledEnum_DISABLED ProductImportProductsPricesRequestPriceAutoActionEnabledEnum = "DISABLED"
	ProductImportProductsPricesRequestPriceAutoActionEnabledEnum_UNKNOWN ProductImportProductsPricesRequestPriceAutoActionEnabledEnum = "UNKNOWN"
	ProductImportProductsPricesRequestPriceAutoActionEnabledEnum_MinPrice ProductImportProductsPricesRequestPriceAutoActionEnabledEnum = "min_price"
)

// 您的价格的货币
type ProductImportProductsPricesRequestPriceCurrencyCodeEnum string
const (
	ProductImportProductsPricesRequestPriceCurrencyCodeEnum_RUB ProductImportProductsPricesRequestPriceCurrencyCodeEnum = "RUB"
	ProductImportProductsPricesRequestPriceCurrencyCodeEnum_CNY ProductImportProductsPricesRequestPriceCurrencyCodeEnum = "CNY"
	ProductImportProductsPricesRequestPriceCurrencyCodeEnum_BYN ProductImportProductsPricesRequestPriceCurrencyCodeEnum = "BYN"
	ProductImportProductsPricesRequestPriceCurrencyCodeEnum_KZT ProductImportProductsPricesRequestPriceCurrencyCodeEnum = "KZT"
	ProductImportProductsPricesRequestPriceCurrencyCodeEnum_EUR ProductImportProductsPricesRequestPriceCurrencyCodeEnum = "EUR' — 欧元。\n  -"
	ProductImportProductsPricesRequestPriceCurrencyCodeEnum_V ProductImportProductsPricesRequestPriceCurrencyCodeEnum = "— 美元。\n  -"
)

// 商品的价格，包括折扣，都显示在商品详情页上
type ProductImportProductsPricesRequestPricePriceEnum string
const (
	ProductImportProductsPricesRequestPricePriceEnum_OldPrice ProductImportProductsPricesRequestPricePriceEnum = "old_price"
	ProductImportProductsPricesRequestPricePriceEnum_Price ProductImportProductsPricesRequestPricePriceEnum = "price"
)

type ProductImportProductsPricesRequestPrice struct {
	CurrencyCode ProductImportProductsPricesRequestPriceCurrencyCodeEnum `json:"currency_code"` // 您的价格的货币。提交的数值必须与你个人主页设置中的货币相匹配。默认情况下，会提交 `RUB` — 俄罗斯卢布。 例如，如果您设置的相互结算的货币是人民币，请提交数值 `CNY`，否则将返回错误。 可填的数值： - `RUB` — 俄罗斯卢布...
	ManageElasticBoostingThroughPrice bool `json:"manage_elastic_boosting_through_price"` // 管理参与“弹性提升”促销活动： - `true`——如果参数 `price` 的数值符合促销活动条件，系统会自动将商品加入促销活动，或在促销活动中提高折扣与提升比例； - `false`——修改参数 `price` 的数值不会影响商品参与促...
	MinPrice string `json:"min_price"` // 应用促销活动后的商品最低价格。<br> [在卖家知识库中了解更多关于最低价格的信息](https://global-help.ozon.com/zh/prices/price-control/)
	MinPriceForAutoActionsEnabled bool `json:"min_price_for_auto_actions_enabled"` // `true`，如果为 Ozon 在添加到促销时会考虑最低价格。如果不传递任何值，价格计算的状态将保持不变。
	Price ProductImportProductsPricesRequestPricePriceEnum `json:"price"` // 商品的价格，包括折扣，都显示在商品详情页上。 若参数`old_price`值大于0,则`price`与`old_price`之间应为具体差额。 差额取决于`price`值。 | `price`值 | 最小差额 | |---|---| | <...
	ProductID int64 `json:"product_id"` // Ozon系统中商品的标识符 — `product_id`。
	NetPrice string `json:"net_price"` // 产品成本价。
	OfferID string `json:"offer_id"` // 卖家系统中的商品标识符是商品货号。
	OldPrice string `json:"old_price"` // 折扣前的价格（在商品卡上划掉）价格以卢布表。小数部分用一个点隔开，点后最多两个字符。 如果商品没有折扣，在这个字段中输入 "0"，并将当前价格提交到 "price"栏中。
	PriceStrategyEnabled ProductImportProductsPricesRequestPricePriceStrategyEnabledEnum `json:"price_strategy_enabled"` // 用于自动应用价格策略的属性： - `ENABLED` — 启用; - `DISABLED` — 关闭; - `UNKNOWN` — 不做任何更改，默认赋值。 如果您之前已启用价格策略的自动应用，并且不希望关闭，请在后续请求中赋值 `UNKN...
	Vat ProductImportProductsPricesRequestPriceVatEnum `json:"vat"` // 商品增值税税率： - `0` — 免除增值税, - `0.05` — 5%, - `0.07` — 7%, - `0.1` — 10%, - `0.2` — 20%, - `0.22` — 22%。 传递当前有效的出价值。
	AutoActionEnabled ProductImportProductsPricesRequestPriceAutoActionEnabledEnum `json:"auto_action_enabled"` // 启用和禁用自动应用活动的属性： - `ENABLED` — 启用; - `DISABLED` — 关闭; - `UNKNOWN` — 不做任何更改，默认赋值。 例如，如果你以前启用了活动的自动应用，并且不想关闭它，请提交`UNKNOWN`。...
}

type ProductImportProductsPricesRequest struct {
	Prices []ProductImportProductsPricesRequestPrice `json:"prices"` // 商品价格信息。
}

// Ozon 上的竞争对手价格。
type PriceIndexesIndexOzonData struct {
	PriceIndexValue float64 `json:"price_index_value"` // 价格指数值。
	MinPrice float64 `json:"min_price"` // Ozon 上竞争对手的最低商品价格。
	MinPriceCurrency string `json:"min_price_currency"` // 价格的货币单位。
}

// 您的商品在其他平台上的价格。
type PriceIndexesIndexSelfData struct {
	MinPrice float64 `json:"min_price"` // 您的商品在其他平台上的最低价格。
	MinPriceCurrency string `json:"min_price_currency"` // 价格的货币单位。
	PriceIndexValue float64 `json:"price_index_value"` // 价格指数值。
}

// 其他平台上的竞争对手价格。
type PriceIndexesIndexExternalData struct {
	MinPrice float64 `json:"min_price"` // 竞争对手在其他平台上的最低商品价格。
	MinPriceCurrency string `json:"min_price_currency"` // 价格的货币单位。
	PriceIndexValue float64 `json:"price_index_value"` // 价格指数值。
}

// 商品价格指数。
// 最终价格指数：
type GetProductInfoPricesResponseItemPriceIndexesColorIndexEnum string
const (
	GetProductInfoPricesResponseItemPriceIndexesColorIndexEnum_WITHOUTINDEX GetProductInfoPricesResponseItemPriceIndexesColorIndexEnum = "WITHOUT_INDEX"
	GetProductInfoPricesResponseItemPriceIndexesColorIndexEnum_SUPER GetProductInfoPricesResponseItemPriceIndexesColorIndexEnum = "SUPER"
	GetProductInfoPricesResponseItemPriceIndexesColorIndexEnum_GREEN GetProductInfoPricesResponseItemPriceIndexesColorIndexEnum = "GREEN"
	GetProductInfoPricesResponseItemPriceIndexesColorIndexEnum_YELLOW GetProductInfoPricesResponseItemPriceIndexesColorIndexEnum = "YELLOW"
	GetProductInfoPricesResponseItemPriceIndexesColorIndexEnum_RED GetProductInfoPricesResponseItemPriceIndexesColorIndexEnum = "RED"
)

type GetProductInfoPricesResponseItemPriceIndexes struct {
	SelfMarketplacesIndexData PriceIndexesIndexSelfData `json:"self_marketplaces_index_data"`
	ColorIndex GetProductInfoPricesResponseItemPriceIndexesColorIndexEnum `json:"color_index"` // 最终价格指数： - `WITHOUT_INDEX` — 无价格指数， - `SUPER` — 超级有利, - `GREEN` — 有利， - `YELLOW` — 中等， - `RED` — 不利。
	ExternalIndexData PriceIndexesIndexExternalData `json:"external_index_data"`
	OzonIndexData PriceIndexesIndexOzonData `json:"ozon_index_data"`
}

// “经济”费率商品。
type FilterWithQuant struct {
	Created bool `json:"created"` // 处于活跃状态的经济商品。
	Exists bool `json:"exists"` // 所有状态下的经济商品。
}

type V1ProductActionTimerStatusResponseStatuses struct {
	ExpiredAt string `json:"expired_at"` // 计时器结束时间。如果该参数为空，则没有有效的计时器。
	MinPriceForAutoActionsEnabled bool `json:"min_price_for_auto_actions_enabled"` // 如果Ozon在添加商品至促销活动时会参考最低价格，则返回值为`true`。
	ProductID int64 `json:"product_id"` // 卖家系统中的商品识别符——`product_id`。
}

type V2GetProductInfoStocksByWarehouseFbsRequestV2 struct {
	Limit int64 `json:"limit"` // 响应中返回的值数量。
	OfferID []string `json:"offer_id"` // 卖家系统中的商品识别码是卖家系统中的商品标识符是商品货号。
	SKU []string `json:"sku"` // Ozon系统中的商品标识符——SKU。
	Cursor string `json:"cursor"` // 后续数据的选择标志。
}

type V2GetProductInfoStocksByWarehouseFbsResponseV2Product struct {
	FreeStock int64 `json:"free_stock"` // 可售商品数量。
	OfferID string `json:"offer_id"` // 卖家系统中的商品识别码是卖家系统中的商品标识符是商品货号。
	Present int64 `json:"present"` // 仓库中的商品总数量。
	ProductID int64 `json:"product_id"` // 卖家系统中的Ozon系统中商品的标识符 — `product_id`。
	Reserved int64 `json:"reserved"` // 仓库中已预留商品的数量。
	SKU int64 `json:"sku"` // Ozon系统中的商品识别码是SKU。
	WarehouseID int64 `json:"warehouse_id"` // 仓库标识符。
	WarehouseName string `json:"warehouse_name"` // 仓库名称。
}

type Productsv1GetProductInfoStocksByWarehouseFbsResponseResult struct {
	ProductID int64 `json:"product_id"` // 卖家系统中的Ozon系统中商品的标识符 — `product_id`。
	Reserved int64 `json:"reserved"` // 仓库中的保留商品的数量。
	WarehouseID int64 `json:"warehouse_id"` // 仓库编号。
	WarehouseName string `json:"warehouse_name"` // 仓库名称。
	SKU int64 `json:"sku"` // Ozon系统中的商品识别码是SKU。
	OfferID string `json:"offer_id"` // 卖家系统中的商品识别码是卖家系统中的商品标识符是商品货号。
	Present int64 `json:"present"` // 库存商品总量。
}

type Productsv1GetProductInfoStocksByWarehouseFbsResponse struct {
	Result []Productsv1GetProductInfoStocksByWarehouseFbsResponseResult `json:"result"` // 该处理方法的结果。
}

// 佣金信息。
type ItemCommissionsv5 struct {
	FBOReturnFlowAmount float64 `json:"fbo_return_flow_amount"` // 退货及取消订单手续费 (FBO)。
	FBSDirectFlowTransMinAmount float64 `json:"fbs_direct_flow_trans_min_amount"` // 干线运输出仓 (FBS)。
	FBSReturnFlowAmount float64 `json:"fbs_return_flow_amount"` // 退货及取消货件佣金，货件处理费用 (FBS)。
	SalesPercentFBO float64 `json:"sales_percent_fbo"` // 销售佣金比例 (FBO)。
	SalesPercentRFBS float64 `json:"sales_percent_rfbs"` // 销售佣金比例 (rFBS)。
	FBODirectFlowTransMinAmount float64 `json:"fbo_direct_flow_trans_min_amount"` // 干线运输出仓 (FBO)。
	FBSDelivToCustomerAmount float64 `json:"fbs_deliv_to_customer_amount"` // 尾程物流 (FBS)。
	FBSDirectFlowTransMaxAmount float64 `json:"fbs_direct_flow_trans_max_amount"` // 干线运输至仓 (FBS)。
	FBSFirstMileMaxAmount float64 `json:"fbs_first_mile_max_amount"` // 货件处理最高佣金 (FBS)。
	FBSFirstMileMinAmount float64 `json:"fbs_first_mile_min_amount"` // 货件处理最低佣金 (FBS)。
	SalesPercentFBP float64 `json:"sales_percent_fbp"` // 销售佣金比例 (FBP)。
	SalesPercentFBS float64 `json:"sales_percent_fbs"` // 销售佣金比例 (FBS)。
	FBODelivToCustomerAmount float64 `json:"fbo_deliv_to_customer_amount"` // 尾程物流 (FBO)。
	FBODirectFlowTransMaxAmount float64 `json:"fbo_direct_flow_trans_max_amount"` // 干线运输至仓 (FBO)。
}

type MarketingAction struct {
	Title string `json:"title"` // 促销活动名称。
	Value int32 `json:"value"` // 促销折扣。
	DateFrom string `json:"date_from"` // 促销活动开始日期与时间。
	DateTo string `json:"date_to"` // 促销活动结束日期与时间。
}

// 营销活动。
// 营销活动
type ItemMarketingActionsEnum string
const (
	ItemMarketingActionsEnum_DateFrom ItemMarketingActionsEnum = "date_from"
	ItemMarketingActionsEnum_DateTo ItemMarketingActionsEnum = "date_to"
	ItemMarketingActionsEnum_Title ItemMarketingActionsEnum = "title"
	ItemMarketingActionsEnum_Value ItemMarketingActionsEnum = "value"
)

type ItemMarketing struct {
	Actions []MarketingAction `json:"actions"` // 营销活动。 `date_from`、`date_to`、`title`和`value`参数会针对每个促销活动分别指定。
}

// 商品价格。
// 您的定价货币（与个人中心设置中指定的货币相同）
type ItemPricev5CurrencyCodeEnum string
const (
	ItemPricev5CurrencyCodeEnum_RUB ItemPricev5CurrencyCodeEnum = "RUB"
	ItemPricev5CurrencyCodeEnum_BYN ItemPricev5CurrencyCodeEnum = "BYN"
	ItemPricev5CurrencyCodeEnum_KZT ItemPricev5CurrencyCodeEnum = "KZT"
	ItemPricev5CurrencyCodeEnum_EUR ItemPricev5CurrencyCodeEnum = "EUR"
	ItemPricev5CurrencyCodeEnum_USD ItemPricev5CurrencyCodeEnum = "USD"
	ItemPricev5CurrencyCodeEnum_CNY ItemPricev5CurrencyCodeEnum = "CNY"
)

type ItemPricev5 struct {
	AutoActionEnabled bool `json:"auto_action_enabled"` // 如果商品启用了促销活动的自动应用，则为 `true`。
	CurrencyCode ItemPricev5CurrencyCodeEnum `json:"currency_code"` // 您的定价货币（与个人中心设置中指定的货币相同）。 可能的值： - `RUB` — 俄罗斯卢布， - `BYN` — 白俄罗斯卢布， - `KZT` — 坚戈， - `EUR` — 欧元， - `USD` — 美元， - `CNY` — 人民...
	MinPrice float64 `json:"min_price"` // 商品在应用所有折扣后的最低价格。
	NetPrice float64 `json:"net_price"` // 商品成本。
	RetailPrice float64 `json:"retail_price"` // 供应商按合同约定的价格。如果没有供货合同，该字段将返回为空。
	Vat float64 `json:"vat"` // 商品的增值税税率。
	MarketingSellerPrice float64 `json:"marketing_seller_price"` // 包含卖家促销的商品价格。
	OldPrice float64 `json:"old_price"` // 未应用折扣前的价格（在商品卡片显示为划线价）。
	Price float64 `json:"price"` // 包含折扣的商品价格（在商品卡片展示的实际售价）。
}

type ProductGetProductInfoPricesV5ResponseItem struct {
	Price ItemPricev5 `json:"price"`
	PriceIndexes GetProductInfoPricesResponseItemPriceIndexes `json:"price_indexes"`
	ProductID int64 `json:"product_id"` // 商品在卖家系统中的标识符 — `product_id`。
	VolumeWeight float64 `json:"volume_weight"` // 商品体积重量。
	Acquiring float64 `json:"acquiring"` // 最高收单手续费。
	Commissions ItemCommissionsv5 `json:"commissions"`
	MarketingActions ItemMarketing `json:"marketing_actions"` // 卖家营销活动。
	OfferID string `json:"offer_id"` // 商品在卖家系统中的标识符 — 货号。
}

type V1ProductActionTimerUpdateRequest struct {
	ProductIds []string `json:"product_ids"` // 卖家系统中的商品识别符列表——`product_id`。
}

type Productv2ProductsStocksRequestStock struct {
	WarehouseID int64 `json:"warehouse_id"` // 得出的仓库编号的方法 [/v1/warehouse/list](#operation/WarehouseAPI_WarehouseList)。
	OfferID string `json:"offer_id"` // 卖家系统中的商品编号是 — 商品代码。
	ProductID int64 `json:"product_id"` // Ozon系统中商品的标识符 — `product_id`。
	Stock int64 `json:"stock"` // 扣除预留库存后的可售商品数量。
}

type Productv2ProductsStocksRequest struct {
	Stocks []Productv2ProductsStocksRequestStock `json:"stocks"` // 仓库中商品的信息。
}

type V4GetProductInfoStocksResponse struct {
	Cursor string `json:"cursor"` // 后续数据的选择标志。
	Items []V4GetProductInfoStocksResponseItem `json:"items"` // 商品信息。
	Total int32 `json:"total"` // 显示库存信息的独特商品数量。
}

type V1GetProductInfoDiscountedRequest struct {
	DiscountedSkus []string `json:"discounted_skus"` // 降价的商品SKU清单。
}

type V1GetProductInfoDiscountedResponseItem struct {
	CommentReasonDamaged string `json:"comment_reason_damaged"` // 对损坏原因的评论。
	Condition string `json:"condition"` // 商品的状态 — 新的或二手的。
	ConditionEstimation string `json:"condition_estimation"` // 商品的状况，以1至7分为标准。 - 1 — 令人满意。 - 2 — 良好。 - 3 — 非常好。 - 4 — 优秀。 - 5-7 — 像新的一样。
	Defects string `json:"defects"` // 商品缺陷。
	DiscountedSKU int64 `json:"discounted_sku"` // 折扣商品的SKU。
	PackageDamage string `json:"package_damage"` // 包装损坏的说明。
	PackagingViolation string `json:"packaging_violation"` // 篡改包装的痕迹。
	Repair string `json:"repair"` // 商品已被修理的痕迹。
	MechanicalDamage string `json:"mechanical_damage"` // 机械性损坏的说明。
	ReasonDamaged string `json:"reason_damaged"` // 损害原因。
	Shortage string `json:"shortage"` // 表示商品不完整。
	SKU int64 `json:"sku"` // 主要商品的SKU。
	WarrantyType string `json:"warranty_type"` // 商品有有效保修的证明。
}

type V1GetProductInfoDiscountedResponse struct {
	Items []V1GetProductInfoDiscountedResponseItem `json:"items"` // 关于减价和主要商品的信息。
}

type V2GetProductInfoStocksByWarehouseFbsResponseV2 struct {
	Cursor string `json:"cursor"` // 用于获取下一批数据的指针。
	HasNext bool `json:"has_next"` // 如果响应中未返回所有商品，则为`true`。
	Products []V2GetProductInfoStocksByWarehouseFbsResponseV2Product `json:"products"` // 商品库存。
}

type Productv2ProductsStocksResponseError struct {
	Message string `json:"message"` // 错误原因。
	Code string `json:"code"` // 错误代码。
}

type V1ProductUpdateDiscountResponse struct {
	Result bool `json:"result"` // 方式工作结果 `true`, 如果正确完成请求。
}

type ProductImportProductsPricesResponseError struct {
	Code string `json:"code"` // 错误代码。
	Message string `json:"message"` // 错误原因。
}

// 按商品曝光度过滤: - `ALL` —— 所有的商品，除了已存档的。 - `VISIBLE` —— 买家可见的商品。 - `INVISIBLE` —— 买家看不到的商品。 - `EMPTY_STOCK`—— 没有现货的商品。 - `NOT_...
type V4Visibility string

// 商品可见性筛选: - `ALL` —— 所有的商品，除了已存档的。 - `VISIBLE` —— 买家可见的商品。 - `INVISIBLE` —— 买家不可见的商品。 - `EMPTY_STOCK` —— 库存未指定的商品。 - `NOT...
type Productv5GetProductListRequestFilterFilterVisibility string

// 商品筛选器。
type Productv5Filter struct {
	OfferID []string `json:"offer_id"` // 基于 `offer_id` 参数的筛选（最多可传递 1000 个值）。
	ProductID []string `json:"product_id"` // 基于 `product_id` 参数的筛选（最多可传递 1000 个值）。
	Visibility Productv5GetProductListRequestFilterFilterVisibility `json:"visibility"`
}

type Productv5GetProductInfoPricesV5Request struct {
	Cursor string `json:"cursor"` // 用于选择数据的指针。
	Filter Productv5Filter `json:"filter"`
	Limit int32 `json:"limit"` // 每页显示的数值数量。
}

type Productv2ProductsStocksResponseResult struct {
	ProductID int64 `json:"product_id"` // Ozon系统中商品的标识符 — `product_id`。
	Updated bool `json:"updated"` // 如果商品信息已被成功更新 — `true`。
	WarehouseID int64 `json:"warehouse_id"` // 仓库编号。
	Errors []Productv2ProductsStocksResponseError `json:"errors"` // 在搜索处理过程中发生的数组错误。
	OfferID string `json:"offer_id"` // 卖家系统中的商品编号是 — 商品代码。
}

// 商品筛选器。
type V4GetProductInfoStocksRequestFilter struct {
	OfferID []string `json:"offer_id"` // 基于参数 `offer_id` 的过滤。 可以提交数值列表。
	ProductID []string `json:"product_id"` // 基于参数 `product_id` 的过滤。 可以提交数值列表。
	Visibility V4Visibility `json:"visibility"`
	WithQuant FilterWithQuant `json:"with_quant"`
}

type ProductImportProductsPricesResponseProcessResult struct {
	Updated bool `json:"updated"` // 如果商品信息已被成功更新 — `true`。
	Errors []ProductImportProductsPricesResponseError `json:"errors"` // 在搜索处理过程中发生的数组错误。
	OfferID string `json:"offer_id"` // 卖家系统中的商品编号是 — 商品代码。
	ProductID int64 `json:"product_id"` // Ozon系统中商品的标识符 — `product_id`。
}

type ProductImportProductsPricesResponse struct {
	Result []ProductImportProductsPricesResponseProcessResult `json:"result"` // 搜索结果。
}

type Productv5GetProductInfoPricesV5Response struct {
	Cursor string `json:"cursor"` // 用于选择数据的指针。
	Items []ProductGetProductInfoPricesV5ResponseItem `json:"items"` // 商品列表。
	Total int32 `json:"total"` // 商品列表中的商品数量。
}

type V4GetProductInfoStocksRequest struct {
	Cursor string `json:"cursor"` // 后续数据的选择标志。
	Filter V4GetProductInfoStocksRequestFilter `json:"filter"`
	Limit int32 `json:"limit"` // 页面上的值数量。最低为1，最高为1000。
}

type V1ProductUpdateDiscountRequest struct {
	ProductID int64 `json:"product_id"` // Ozon系统中商品的标识符 — `product_id`。
	Discount int32 `json:"discount"` // 折扣力度：从3%到99%。
}

type Productsv1GetProductInfoStocksByWarehouseFbsRequest struct {
	SKU []string `json:"sku"` // Ozon系统中的商品识别码是SKU。
	OfferID []string `json:"offer_id"` // 卖家系统中的商品识别码是卖家系统中的商品标识符是商品货号。
}

type V1ProductActionTimerStatusResponse struct {
	Statuses []V1ProductActionTimerStatusResponseStatuses `json:"statuses"`
}

// 搜索结果。
type Productv2ProductsStocksResponse struct {
	Result []Productv2ProductsStocksResponseResult `json:"result"`
}

type V1ProductActionTimerStatusRequest struct {
	ProductIds []string `json:"product_ids"` // 卖家系统中的商品识别符列表——`product_id`。
}
