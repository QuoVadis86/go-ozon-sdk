...
// 第一英里 FBS。
// FirstMileType values
type FirstMileType string
const (
	FirstMileTypeDropOff FirstMileType = "DropOff"
	FirstMileTypePickup FirstMileType = "Pickup"
)
...
type WarehouseFirstMileType struct {
	...
	FirstMileType FirstMileType `json:"first_mile_type"` // 第一英里的类型 — `DropOff` 或 `Pickup`。
}
...
const (
	...
	StatusDisabledDueToLimit Status = "disabled_due_to_limit"
...
type WarehouseListResponseWarehouse struct {
	...
	Status Status `json:"status"` // 仓库状况。 仓库状态与个人账户中的状态的对应关系: | 状态 Seller&nbsp;API | 个人账户中的状态 | |---|---| | `new` | 正在激活 | | `created` | 已激活 | | `disabled` ...
...
const (
	...
	WorkingDaysSunday WorkingDays = "SUNDAY" // 星期天。
)
...
const (
	TPLIntegrationTypeAggregator TPLIntegrationType = "aggregator"
	TPLIntegrationTypeV3plTracking TPLIntegrationType = "3pl_tracking"
	TPLIntegrationTypeNonIntegrated TPLIntegrationType = "non_integrated"
	TPLIntegrationTypeHybrid TPLIntegrationType = "hybrid"
)
