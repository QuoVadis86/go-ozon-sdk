...
type DeliveryMethodListRequestFilter struct {
	...
	Status string `json:"status"` // 快递方式状态: - `NEW` — 已创建, - `EDITED` — 正在编辑, - `ACTIVE` — 已激活, - `DISABLED` — 未激活。
...
type DeliveryMethodListV2ResponseDeliveryMethod struct {
	...
	Status string `json:"status"` // 配送方式状态： - `NEW`：已创建， - `EDITED`：编辑中， - `ACTIVE`：已启用， - `DISABLED`：未启用， - `WAITING`：审核中， - `BROKEN`：异常。
	TPLIntegrationType string `json:"tpl_integration_type"` // 与配送服务的集成类型： - `aggregator`：由第三方配送，Ozon负责登记订单； - `3pl_tracking`：由第三方配送，卖家负责登记订单； - `non_integrated`：由卖家自行配送； - `hybrid`：混...
